<?php
/**
 * Plugin Name: Comment Word Blacklist Manager
 ...
 * Version:     1.0.0
 * Author:      Team GeekParadize
 * Author URI:  https://www.geekparadize.fr
 * Text Domain: comment-word-blacklist-manager
 * Domain Path: /languages
 * License:     GPL
 */

if( ! defined( 'CWBL_MANAGER_BASE ' ) ) {
	define( 'CWBL_MANAGER_BASE', plugin_basename(__FILE__) );
}

if( ! defined( 'CWBL_MANAGER_DIR' ) ) {
	define( 'CWBL_MANAGER_DIR', plugin_dir_path( __FILE__ ) );
}

if( ! defined( 'CWBL_MANAGER_VER' ) ) {
	define( 'CWBL_MANAGER_VER', '1.0.0' );
}


class CWBL_Manager_Core
{
	...
	public function textdomain() {
		...
		load_plugin_textdomain( 'comment-word-blacklist-manager', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
	}
	...
	public function load_settings() {
		...
		add_settings_field( 'blacklist-word-source', __( 'Blacklist Source', 'comment-world-blacklist-manager' ), array( $this, 'source_field' ), 'discussion', 'default' );

		// load the custom list field
		add_settings_field( 'blacklist-word-local', __( 'Local Blacklist', 'comment-world-blacklist-manager' ), array( $this, 'local_field' ), 'discussion', 'default' );

		// load the exclusion field
		add_settings_field( 'blacklist-word-exclude', __( 'Excluded Terms', 'comment-world-blacklist-manager' ), array( $this, 'exclude_field' ), 'discussion', 'default' );
	...
	public function source_field() {
		...
		echo '<fieldset>';
			echo '<legend class="screen-reader-text"><span>' . __( 'Blacklist Source', 'comment-word-blacklist-manager' ) . '</span>';
		echo '</legend>';

		echo '<p>';
			echo '<label>' . __( 'Data from the sources below will be loaded into the comment blacklist automatically.', 'comment-word-blacklist-manager' ) . '</label>';
		echo '</p>';
		...
		if ( ! $sources || empty( $sources ) ) {
			echo '<p class="description">' . __( 'No blacklist sources have been defined.', 'comment-word-blacklist-manager' ) . '</p>';
		}
		...
		foreach( (array) $sources as $source ) {
			...
			echo '<li><a class="imgedit-help-toggle" href="' . esc_url( $source ) . '" title="' . __( 'View external source', 'comment-word-blacklist-manager' ) . '" target="_blank"><span class="dashicons dashicons-external"></span></a>&nbsp;' . esc_url( $source ) . '</li>';
		...
		echo '<a class="button button-secondary" href=" ' . admin_url( 'options-discussion.php' ) . '?CWBLm-update=manual">' . __( 'Run manual update', 'comment-blacklist-manager' ) . '</a>';
	...
	public function local_field() {
		...
		echo '<fieldset>';
			echo '<legend class="screen-reader-text"><span>' . __( 'Local Blacklist', 'comment-word-blacklist-manager' ) . '</span>';
		echo '</legend>';

		echo '<p>';
			echo '<label for="blacklist_local">' . __( 'Any terms entered below will be added to the data retrieved from the blacklist sources. One word or IP per line. It will match inside words, so &#8220;press&#8221; will match &#8220;WordPress&#8221;.', 'comment-word-blacklist-manager' ) . '</label>';
		echo '</p>';
	...
	public function exclude_field() {
		...
		echo '<fieldset>';
			echo '<legend class="screen-reader-text"><span>' . __( 'Excluded Terms', 'comment-word-blacklist-manager' ) . '</span>';
		echo '</legend>';

		echo '<p>';
			echo '<label for="blacklist_exclude">' . __( 'Any terms entered below will be excluded from the blacklist updates. One word or IP per line. It will match inside words, so &#8220;press&#8221; will match &#8220;WordPress&#8221;.', 'comment-word-blacklist-manager' ) . '</label>';
		echo '</p>';
	...
	public function update_blacklist_manual() {
		...
		if ( ! isset( $_REQUEST['CWBLm-update'] ) || isset( $_REQUEST['CWBLm-update'] ) && $_REQUEST['CWBLm-update'] !== 'manual' ) {
			return;
		}
		...
		$redirect	= admin_url( 'options-discussion.php' ) . '?CWBLm-update=success';
	...
	public function manual_update_notice() {
		...
		// check for our query string
		if ( ! isset( $_REQUEST['CWBLm-update'] ) || isset( $_REQUEST['CWBLm-update'] ) && $_REQUEST['CWBLm-update'] !== 'success' ) {
			return;
		}

		echo '<div id="setting-error-settings_updated" class="updated settings-error">';
			echo '<p><strong>' . __( 'Blacklist terms were updated successfully.', 'comment-word-blacklist-manager' ) . '</strong></p>';
		echo '</div>';
	...
	static function blacklist_process_loader() {
		...
		// set our transient
		set_transient( 'blacklist_update_process', 1, apply_filters( 'CWBLm_update_schedule', 60*60*24 ) );
	...
	static function blacklist_sources() {
		...
		$lists	= array(
			'https://raw.githubusercontent.com/geekparadize/Comment-Word-Blacklist-Manager/master/blacklist.txt'
		);

		return apply_filters( 'CWBLm_sources', (array) $lists );
	...
	static function parse_data_source( $source ) {
		...
		$result	= apply_filters( 'CWBLm_parse_data_result', $result, $source );
...
$CWBL_Manager_Core = CWBL_Manager_Core::getInstance();
